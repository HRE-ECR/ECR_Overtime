...
export default function ResetPassword() {
  ...
  // When the user clicks the reset email link, Supabase emits PASSWORD_RECOVERY
  useEffect(() => {
      ...
      <div className="rounded-3xl bg-slate-900/60 border border-slate-800 shadow-card p-6">
        ...
        <form onSubmit={submit} className="mt-6 grid gap-3">
          ...
          <input value={pw1} onChange={(e) => setPw1(e.target.value)} type="password" required
            className="w-full px-4 py-3 rounded-2xl bg-slate-950/40 border border-slate-700 text-white" autoComplete="new-password" />

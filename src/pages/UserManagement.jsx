...
export default function UserManagement({ testMode, onTestModeChange }) {
  ...
  const load = async () => {
    ...
    const {data,error:err} = await supabase.from('profiles').select('id, full_name, role, created_at').order('full_name',{ascending:true})
  ...
  const loadStaff = async (uid) => {
    const {data} = await supabase.from('user_staffing').select('team, band').eq('user_id', uid).maybeSingle()
    setTeam(data?.team||'')
    setBand(data?.band||'')
  }

  useEffect(()=>{load()},[])
  useEffect(()=>{ if(selected){ setRole(selected.role); loadStaff(selected.id) } },[selectedId])
  ...
  const saveStaffing = async () => {
    setError('')
    const {error:err} = await supabase.from('user_staffing').upsert({user_id:selectedId, team: team||null, band: band||null},{onConflict:'user_id'})
    if(err) setError(err.message); else loadStaff(selectedId)
  }

  const labelForUser = (u) => `${(u.full_name||u.id)} (${u.role})`

  const saveTestMode = async (value) => {
    setError('')
    const {error:err} = await supabase.from('app_settings').upsert({id:1, test_mode_enabled:value},{onConflict:'id'})
    if(err) setError(err.message)

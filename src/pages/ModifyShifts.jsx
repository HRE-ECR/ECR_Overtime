...
export default function ModifyShifts(){
  ...
  const updateSlots=async(id,v)=>{
    ...
    if(Number.isNaN(slots)||slots<0){setError('Slots must be 0 or greater.');return;}
    const{error:err}=await supabase.from('shifts').update({spots_available:slots}).eq('id',id)
  ...
  const del=async(id)=>{
    ...
    const{error:err}=await supabase.rpc('delete_shift_and_decline',{p_shift_id:id})
    if(err) setError(err.message); else load()
  }
    ...
    <div>
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <label className="text-xs text-slate-300 flex items-center gap-2"><input type="checkbox" checked={showDeleted} onChange={(e)=>setShowDeleted(e.target.checked)}/>Show deleted</label>
          ...
          <div key={s.id} className="rounded-3xl bg-slate-900/50 border border-slate-800 p-4 shadow-card">
            <div className="flex items-start justify-between gap-3 flex-wrap">
              ...
              <div className="flex flex-col gap-2 items-end">
                ...
                {s.shift_status!=='deleted' ? <button onClick={()=>del(s.id)} className="px-3 py-2 rounded-xl bg-rose-500/20 border border-rose-500/30 text-rose-100 font-extrabold text-xs">Delete shift</button> : null}
              </div>
            </div>
          </div>
        ))}

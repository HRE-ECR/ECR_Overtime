...
function dayOfWeek(isoDate){return new Date(isoDate+'T00:00:00').getDay()}
    ...
    <div>
      ...
      <div className="mt-5 rounded-3xl bg-slate-900/60 border border-slate-800 shadow-card p-5">
        <div className="grid md:grid-cols-2 gap-3">
          ...
          <div><label className="text-xs font-bold text-slate-300">Department</label><input value={department} onChange={(e)=>setDepartment(e.target.value)} type="text" className="mt-1 w-full px-4 py-3 rounded-2xl bg-slate-950/40 border border-slate-700 text-white"/></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="text-xs font-bold text-slate-300">Day slots</label><input value={daySlots} onChange={(e)=>setDaySlots(e.target.value)} type="number" min="0" className="mt-1 w-full px-4 py-3 rounded-2xl bg-slate-950/40 border border-slate-700 text-white"/></div>
            <div><label className="text-xs font-bold text-slate-300">Night slots</label><input value={nightSlots} onChange={(e)=>setNightSlots(e.target.value)} type="number" min="0" className="mt-1 w-full px-4 py-3 rounded-2xl bg-slate-950/40 border border-slate-700 text-white"/></div>
          </div>
        </div>

...
export default function AvailableShifts(){
  ...
  const myReqByShift=useMemo(()=>{
    const m={};
    for(const r of myRequests) m[r.shift_id]=r;
    return m;
  },[myRequests])
  ...
  const loadRoster=async()=>{
    ...
    if(team){
      const {data}=await supabase.from('team_roster_pattern').select('day_index, roster_type').eq('team',team).order('day_index',{ascending:true})
      setPattern(data||[])
    } else setPattern([])
  }
  ...
  const load=async()=>{
    ...
    const [{data:shiftData,error:sErr},{data:myData,error:mErr},{data:cntData,error:cErr}] = await Promise.all([
      supabase.from('shifts').select('id, shift_date, shift_type, start_time, end_time, spots_available, department, notes, shift_status')
        .gte('shift_date', today).eq('shift_status','active')
        .order('shift_date',{ascending:true}).order('shift_type',{ascending:true}),
      supabase.from('ot_requests').select('id, shift_id, user_id, status, requested_at, decided_at, archived, notes')
        .eq('user_id', userId).eq('archived', false),
      supabase.from('ot_shift_counts').select('shift_id, requested, approved, declined')
    ])

    if(sErr||mErr||cErr){setError(sErr?.message||mErr?.message||cErr?.message||'Failed to load');setLoading(false);return;}
    const map={};
  ...
  useEffect(()=>{loadRoster();load()},[])
  useEffect(()=>{localStorage.setItem('oh_showOnlyRest', showOnlyRest?'1':'0')},[showOnlyRest])

  const isRest=(dateIso)=>{
  ...
  const visible=useMemo(()=>{
    ...
    return shifts.filter(s=>isRest(s.shift_date))
  ...
  const requestOt=async(shiftId)=>{
    ...
    const {data:updated,error:upErr} = await supabase.from('ot_requests')
      .update({status:'requested', requested_at:new Date().toISOString(), decided_at:null, decided_by:null, archived:false})
      .eq('shift_id',shiftId).eq('user_id',userId).select('id')

    if(upErr){setError(upErr.message);return;}
    if(!updated||updated.length===0){
      const {error:insErr}=await supabase.from('ot_requests').insert({shift_id:shiftId,user_id:userId,status:'requested',requested_at:new Date().toISOString(),archived:false})
  ...
  const cancelRequest=async(shiftId)=>{
    setError('')
    const userId=(await supabase.auth.getUser()).data?.user?.id
    const {error:err}=await supabase.from('ot_requests').update({status:'cancelled'}).eq('shift_id',shiftId).eq('user_id',userId)
    if(err) setError(err.message); else load()
  }

  const openNotes=(shiftId)=>{
    const req=myReqByShift[shiftId]
    if(!req){setError('Request the shift first to add notes.');return;}
  ...
  const saveNotes=async()=>{
    ...
    setNotesSaving(true)
    const {error:err}=await supabase.from('ot_requests').update({notes: trimmed}).eq('shift_id',notesShiftId).eq('user_id',userId)
    ...
    <div>
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div className="min-w-0">
          ...
          {myTeam ? <p className="text-xs text-slate-400 mt-1">Roster filter team: <span className="text-slate-200 font-bold">{myTeam}</span></p> : <p className="text-xs text-amber-200 mt-1">Team not set — set it in User Profile to enable rest-day filtering.</p>}
        </div>
      ...
      {notesOpen ? (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="w-full max-w-lg rounded-3xl bg-slate-900 border border-slate-700 shadow-card p-5">
            ...
            <textarea value={notesText} onChange={(e)=>setNotesText(e.target.value.slice(0,500))} rows={6}
              className="mt-3 w-full px-4 py-3 rounded-2xl bg-slate-950/40 border border-slate-700 text-white" placeholder="Enter notes…" />
            ...
            <div className="mt-4 flex gap-2">
              ...
              <button disabled={notesSaving} onClick={saveNotes} className="flex-1 px-4 py-3 rounded-2xl bg-white text-navy-900 font-extrabold disabled:opacity-60">{notesSaving?'Saving…':'Save notes'}</button>
            </div>
          </div>
        </div>
      ) : null}

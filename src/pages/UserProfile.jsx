...
function normalizeName(input){
  ...
  surname = surname.split(' ').map(part=>part.split('-').map(seg=>seg?seg[0].toUpperCase()+seg.slice(1).toLowerCase():seg).join('-')).join(' ')
  return `${initial}. ${surname}`
}

function isValidName(v){ return /^[A-Z]\.\s+[A-Za-z][A-Za-z\s\-']+$/.test(v) }
...
export default function UserProfile(){
  ...
  const save=async()=>{
    ...
    if(!isValidName(fixed)){setError('Name must be in the format “J. Surname” (example: “J. Edwards”).');return;}
    setSaving(true)
    const userId=(await supabase.auth.getUser()).data?.user?.id
    const {error:err}=await supabase.from('profiles').update({full_name:fixed, phone: phone || null}).eq('id', userId)
    if(err){setError(err.message);setSaving(false);return;}
    if(role==='employee'){
      const {error:sErr}=await supabase.from('user_staffing').upsert({user_id:userId, team: team||null, band: band||null},{onConflict:'user_id'})
        ...
        {loading ? <div className="text-slate-300">Loading…</div> : (
          <div className="grid gap-4">
            <div>
              ...
              <input value={fullName} onChange={(e)=>setFullName(e.target.value)} placeholder="J. Surname" type="text" className="mt-1 w-full px-4 py-3 rounded-2xl bg-slate-950/40 border border-slate-700 text-white"/>
            </div>
            <div>
              <label className="text-xs font-bold text-slate-300">Contact number (optional)</label>
              <input value={phone} onChange={(e)=>setPhone(e.target.value)} placeholder="e.g. 07…" type="tel" className="mt-1 w-full px-4 py-3 rounded-2xl bg-slate-950/40 border border-slate-700 text-white"/>
            </div>
            {role==='employee' ? (
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-950/20 p-3 text-slate-200 text-sm">You are a <b>manager</b>. Team/Band are not required.</div>
            )}

            <button disabled={saving} onClick={save} className="px-4 py-3 rounded-2xl bg-white text-navy-900 font-extrabold disabled:opacity-60">{saving?'Saving…':'Save profile'}</button>
          </div>
        )}
      </div>
    </div>
